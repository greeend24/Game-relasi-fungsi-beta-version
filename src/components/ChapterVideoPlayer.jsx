 ...
 * - Restricted controls: Play/Pause, Restart, Rewind 5s (Seek forward/skip completely disabled)
...
export default function ChapterVideoPlayer({
  ...
  const handleProgressBarClick = (e) => {
    ...
    // RESTRICTION: STRICTLY NO JUMPING FORWARD! Only allow seeking within watched territory
    if (targetTime > maxWatchedTime + 0.5) {
      try { audioEngine.playError?.(); } catch {}
      setSkipWarning('⚠️ Tidak dapat melompat ke depan. Tonton video secara berurutan!');
  ...
  const handleSeeking = () => {
    if (!videoRef.current) return;
    // If native seek tries to skip ahead beyond maxWatchedTime, lock it back
  ...
  const currentActiveQuiz = (isRetryActive && videoData?.quiz?.retryQuestion)
    ? { ...videoData.quiz, ...videoData.quiz.retryQuestion }
    : videoData?.quiz;
  ...
  const handleQuizAnswer = (option) => {
    ...
    if (isCorrect) {
      ...
      // Play Benar Audio
      const audioBenar = videoData?.quiz?.audioBenar;
      const universalBenar = UNIVERSAL_AUDIO.benar.audio;
      const message = videoData?.quiz?.textBenar || UNIVERSAL_AUDIO.benar.text;
      ...
      // Play Salah Audio
      const audioSalah = videoData?.quiz?.audioSalah;
      const universalSalah = UNIVERSAL_AUDIO.salah.audio;
      const message = videoData?.quiz?.textSalah || UNIVERSAL_AUDIO.salah.text;
  ...
  return (
    <div className="relative w-full h-full flex flex-col justify-between overflow-hidden select-none bg-slate-950/70 rounded-2xl sm:rounded-3xl border border-white/20 shadow-2xl backdrop-blur-md">
      {/* ── TOP HEADER BAR ── */}
      <div className="flex items-center px-4 sm:px-6 py-2.5 bg-black/40 border-b border-white/10 z-20 flex-shrink-0">
        <h2 className="text-sm sm:text-base font-black text-white/90 truncate font-pencil tracking-wide">
          {videoData.title}
        </h2>
      </div>
      ...
      >
        ...
        {isQuizOpen && videoData.quiz && (
          ...
          >
            <div className="w-full max-w-xl glass-panel glass-sheen rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex flex-col gap-3 shadow-2xl border border-white/40 max-h-full overflow-y-auto">
              {/* Quiz Header */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {(currentActiveQuiz?.options || []).map((opt, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left font-bold text-sm sm:text-base flex items-center justify-between transition-all cursor-pointer ${optStyle}`}
                    >
                      <div className="flex items-center gap-2">
                      ...
                      {quizStatus === 'correct' && isThisCorrect && <CheckCircle2 className="w-5 h-5 text-white" />}
                      {quizStatus === 'wrong' && isChosen && <XCircle className="w-5 h-5 text-white" />}
                    </button>
                  );
                })}
              </div>
      ...
      {!isQuizOpen && (
        ...
        >
          ...
          <div 
            ...
            title="Klik untuk memundurkan video (tidak bisa skip ke depan)"
          >
            ...
            {currentTime > 0 && (
              <div 
                className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-amber-300 border-2 border-white shadow-md transition-all pointer-events-none"
