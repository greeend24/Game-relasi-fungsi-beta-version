...
const OUTPUT_DIR = 'D:\\File Penting\\File S2\\Thesis Project\\Video\\Snowy';
const DEFAULT_DURATION_SEC = 300; // 5 minutes (300 seconds)
const FPS = 60;

const SCENES = [
  { timeMode: 'pagi', fileName: '1_langit_snowy_pagi_1080p60.mp4' },
  { timeMode: 'siang', fileName: '2_langit_snowy_siang_1080p60.mp4' },
  { timeMode: 'sore', fileName: '3_langit_snowy_sore_1080p60.mp4' },
  { timeMode: 'malam', fileName: '4_langit_snowy_malam_1080p60.mp4' }
];
...
app.whenReady().then(async () => {
  console.log('====================================================');
  console.log('SNOWY SKY VIDEO EXPORT PIPELINE (1080p 60FPS MP4)');
  console.log('Snowflake Edition with High-Res Particle Assets');
  ...
  await win.loadFile(path.join(__dirname, 'export_snowy_renderer.html'));
  ...
  function runNextJob() {
    if (currentJobIndex >= jobsToRun.length) {
      ...
      console.log('\n====================================================');
      console.log(`🎉 ALL ${jobsToRun.length} SNOWY SCENES EXPORTED SUCCESSFULLY IN ${overallElapsed} MINUTES!`);
    ...
    console.log(`\n----------------------------------------------------`);
    console.log(`>>> [${currentJobIndex + 1}/${jobsToRun.length}] EXPORTING SNOWY SCENE: ${job.timeMode.toUpperCase()}`);
    console.log(`File: ${job.fileName}`);
    console.log(`----------------------------------------------------`);

    ipcMain.once('job-done', (event, data) => {
      console.log(`✓ FINISHED: Snowy Scene "${job.timeMode}" exported in ${data.totalTimeSec}s`);
      currentJobIndex++;
      runNextJob();
    });
